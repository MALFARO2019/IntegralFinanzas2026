...
import { Wallet, Plus, ArrowUpCircle, ArrowDownCircle, Landmark, Banknote, PiggyBank, CreditCard } from 'lucide-react'
import { usePreferences } from '@/lib/preferences-context'
import { SettingsSelector } from '@/components/settings-selector'
import { AddAccountModal } from '@/components/add-account-modal'
import type { Account } from '@/lib/data'

const ACCOUNT_TYPE_META: Record<string, { label: string; icon: typeof Banknote; emoji: string }> = {
    CHECKING: { label: 'Cuenta Corriente', icon: Banknote, emoji: '🏦' },
    SAVINGS: { label: 'Ahorros', icon: PiggyBank, emoji: '🐷' },
    CREDIT: { label: 'Tarjeta Crédito', icon: CreditCard, emoji: '💳' },
    INVEST: { label: 'Inversiones', icon: Landmark, emoji: '📈' },
    CASH: { label: 'Efectivo', icon: Banknote, emoji: '💵' },
    LOAN: { label: 'Préstamos', icon: Landmark, emoji: '🏛️' },
}
...
export default function AccountsPageClient({ accounts, netWorth, assets, debt }: Props) {
    ...
    // Agrupar por tipo (todos los tipos, no solo los 4 anteriores)
    const allTypes = [...new Set(accounts.map(a => a.type))]
    const grouped = allTypes.map(key => {
        const meta = ACCOUNT_TYPE_META[key] ?? { label: key, icon: Landmark, emoji: '🏦' }
        const items = accounts.filter(a => a.type === key)
        const total = items.reduce((s, a) => s + (a.openingBalance ?? 0), 0)
    ...
    return (
        <div className="flex flex-col min-h-screen pb-24 lg:pb-0">
            ...
            <div className="sticky top-0 z-30 bg-background/90 backdrop-blur-lg border-b border-border/50 px-4 pt-4 pb-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-primary-foreground rounded-xl text-xs font-bold hover:bg-primary/90 transition-all"
                    >
            ...
            <div className="px-4 py-4 space-y-4">
                ...
                {accounts.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-muted-foreground text-xs mt-1 mb-4">Agrega tu primera cuenta para comenzar</p>
                        <button
                            onClick={() => setShowAdd(true)}
                            className="px-6 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:bg-primary/90 transition-all"
                        >
                            + Agregar Cuenta
                        </button>
                    </div>
                ...
                {/* Lista por tipo */}
                {grouped.map(({ key, label, emoji, items, total }) => (
                    <div key={key}>
                        ...
                        <div className="bg-card border border-border/40 rounded-2xl overflow-hidden divide-y divide-border/30">
                            {items.map(acc => (
                                <div key={acc.id} className="flex items-center gap-3 px-4 py-3.5 hover:bg-secondary/20 transition-colors">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-xs font-extrabold ${key === 'CREDIT' || key === 'LOAN' ? 'bg-destructive/15 text-destructive' :
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[10px] text-muted-foreground">
                                            {acc.institution ? `${acc.institution} · ` : ''}{acc.currencyCode}
                                        </p>
                                    </div>
            ...
            <button
                onClick={() => setShowAdd(true)}
                className="fixed bottom-24 right-6 w-14 h-14 bg-primary text-primary-foreground rounded-2xl shadow-lg shadow-primary/30 flex items-center justify-center hover:scale-105 active:scale-95 transition-all z-40 lg:hidden"
            >
                <Plus size={26} />
            </button>
